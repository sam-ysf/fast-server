...
namespace fserv {
    ...
    class TimeoutTimer {
        using ClientKeyUuid = std::pair<StackNode<KeyType>*, std::uint64_t>;
    public:
        ...
        void run(
            ...
            const std::function<void(const std::vector<ClientKeyUuid>&)>&
                handle_timeout)
        {
            if (timeout_interval > 0) {
                ...
                worker_ = std::make_unique<std::jthread>([this,
                                                          ...
                                                          generation_tokens,
                                                          handle_timeout]() {
                    do_run(timeout_interval, generation_tokens, handle_timeout);
                });
            }
        }
    ...
    private:
        ...
        void do_run(
            unsigned timeout_interval,
            std::vector<std::atomic<ClientGenerationToken>>* generation_tokens,
            std::function<void(const std::vector<ClientKeyUuid>&)>
                handle_timeout)
        {
            ...
            while (true) {
                ...
                std::vector<ClientKeyUuid> timed_out_keys;
            ...
            std::unordered_map<StackNode<KeyType>*, std::uint64_t>& uuids)
        {
            ...
            for (const auto& [key, then]: keys) {
                ...
                if (time_delta > timeout_interval) {
                    ...
                    auto [writing, expected_uuid, _1] = token.load();

                    if (static_cast<bool>(writing)) {
                        continue;
                    }
                    ...
                    if (token.compare_exchange_strong(expected, next)) {
                        timed_out_keys.push_back(
                            std::make_pair(key, expected_uuid));
                    }
                }
            }
