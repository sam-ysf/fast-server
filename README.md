...
This generates `fserv` (a simple echo server) that can be run from the command line and listens on port 60007 by default. The server writes an editable configuration file (`~/.config/fserv/server/config.json`) that can be modified with different network parameters.
