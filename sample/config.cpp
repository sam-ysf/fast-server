/* config.cpp -- v1.0 */

#include "config.hpp"
#include "json.hpp"
#include <dirent.h>
#include <filesystem>
#include <fstream>
#include <string>
#include <sys/stat.h>
#include <sys/types.h>
#include <vector>

namespace {

    template <typename T>
    inline void maybe_load_field(T&, const std::string&, const nlohmann::json&);

    template <>
    inline void maybe_load_field<std::string>(std::string& value,
                                              const std::string& key,
                                              const nlohmann::json& source)
    {
        auto j = source.find(key);
        if (j != source.end()) {
            const auto& v = j.value();
            if (v.is_string()) {
                value = static_cast<std::string>(v);
            }
        }
    }

    inline void save_config(const char* path, const app::Config& config)
    {
        nlohmann::json json_config;

        if (const auto server_port = config["server-port"]; server_port) {
            json_config["server-port"] = server_port.value();
        }

        if (const auto max_concurrent_connections
            = config["max-concurrent-connections"];
            max_concurrent_connections) {
            json_config["max-concurrent-connections"]
                = max_concurrent_connections.value();
        }

        if (const auto max_workers = config["max-workers"]; max_workers) {
            json_config["max-workers"] = max_workers.value();
        }

        if (const auto timeout_interval = config["timeout-interval"];
            timeout_interval) {
            json_config["timeout-interval"] = timeout_interval.value();
        }

        if (std::ofstream ofs(path); ofs.is_open()) {
            ofs << json_config.dump(2);
        }
    }
} // namespace

app::Config app::load_config(const char* path)
{
    static const std::string kDefaultPort = "60007";
    static const std::string kDefaultMaxConcurrentConnections = "50000";
    static const std::string kDefaultMaxWorkers = "2";

    Config config;

    // Load config defaults
    config.global_params["server-port"] = kDefaultPort;
    config.global_params["max-concurrent-connections"]
        = kDefaultMaxConcurrentConnections;
    config.global_params["max-workers"] = kDefaultMaxWorkers;

    // Create configuration directory (if not already exists)
    std::filesystem::path config_file_path(path);
    if (std::filesystem::path parent_dir_path = config_file_path.parent_path();
        !std::filesystem::exists(parent_dir_path)) {
        std::filesystem::create_directories(parent_dir_path);
    }

    // Load config from file
    std::ifstream ifs(config_file_path);
    if (!ifs.is_open()) {
        return save_config(config_file_path.c_str(), config), config;
    }

    nlohmann::json loaded_config;

    try {
        loaded_config = nlohmann::json::parse(ifs);
        ifs.close();
    } catch (nlohmann::detail::parse_error& e) {
        std::fprintf(stderr, "[err] %s\n", e.what());
        ifs.close();
        save_config(config_file_path.c_str(), config);
        return config;
    }

    // Specifies the echo server listener port
    maybe_load_field(
        config.global_params["server-port"], "server-port", loaded_config);

    // Specifes the maximum number of concurrent connections
    maybe_load_field(config.global_params["max-concurrent-connections"],
                     "max-concurrent-connections",
                     loaded_config);

    // Specifies the number of concurrently active client jobs
    maybe_load_field(
        config.global_params["max-workers"], "max-workers", loaded_config);

    // Specifies the
    maybe_load_field(config.global_params["timeout-interval"],
                     "timeout-interval",
                     loaded_config);

    // Unencrypted or TLS; will use unencrypted if not specified
    maybe_load_field(config.global_params["network-protocol"],
                     "network-protocol",
                     loaded_config);

    // Ssl certificate file, ignored if tls not specified
    maybe_load_field(config.global_params["ssl-certificate"],
                     "ssl-certificate",
                     loaded_config);
    // Ssl certificate file password
    maybe_load_field(config.global_params["ssl-certificate-password"],
                     "ssl-certificate-password",
                     loaded_config);

    // Private key file, ignored if tls not specified
    maybe_load_field(config.global_params["ssl-private-key"],
                     "ssl-private-key",
                     loaded_config);

    // Private key file password
    maybe_load_field(config.global_params["ssl-private-key-password"],
                     "ssl-private-key-password",
                     loaded_config);

    // Done
    return config;
}
